<template>
  <view class="cmd-bottom-nav" :class="[textAuto ? 'cmd-bottom-nav-text-auto':'',fixed ? 'cmd-bottom-nav-fixed':'']"
    :style="setColorStyle">
    <view class="line" style="position: absolute;top: 0;"></view>
    <view :class="['cmd-bottom-nav-box',select == index ? 'cmd-bottom-nav-active':'']" :style="select == index ? setActiveFontColorStyle : ''" 
      ...
      <view class="cmd-bottom-nav-box-icon" v-if="nav.barBargea === true">
        <view class="bar-bargea"></view>
        <!-- <uni-icon v-if="nav.icon && !nav.src" :type="nav.icon" size="24" :color="select == index ?activeFontColor :  fontColor"></uni-icon> -->
      </view>
      <!-- <image v-if="nav.src && !nav.icon" class="cmd-bottom-nav-box-img" :src="nav.src"
...
<style>
  ...
  .cmd-bottom-nav-box-icon .bar-bargea {
    position: absolute;
    top: -7upx;
    right: -23upx;
    width: 10px;
  ...
  .cmd-bottom-nav-box.cmd-bottom-nav-active .cmd-bottom-nav-box-img {
    /* opacity: 1; */
    animation: scaleDraw;/*动画名称*/  
    animation-duration: 1.5s;/*动画持续时间*/  
    -webkit-animation:scaleDraw 1.5s;/*针对webkit内核*/
  ...
  .cmd-bottom-nav-box.cmd-bottom-nav-active .cmd-bottom-nav-box-text {
    color: #333333;
    font-family: "Source Han Sans CN";
  }
