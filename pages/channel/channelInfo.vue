    ...
    >
      <view class="from">
        <view class="from-content">
          <view class="left"> 商家名称<text class="text"></text> </view>
          <view class="right">
            <input
              type="text"
              class="input-style"
          ...
          <view class="right">
            <view class="select-content">
              <picker
                ...
                range-key="areaname"
              >
                <view class="uni-input" v-if="index < 0" style="color: #adadad"
                  >所在区域</view
                >
                <view class="uni-input" v-if="regionList.length > 0">{{
                  regionList[index] ? regionList[index].areaname : ""
                }}</view>
        ...
        <view class="from-content">
          <view class="left left1"> 业务记录<text class="text"></text> </view>
          <view class="right">
      ...
      <view class="add-cancel-style">
        ...
        <view class="add-btn-style" style="margin-right: 20upx" @tap="add(1)"
          >提交确认</view
        >
    ...
    goToPage(page) {
      ...
      if (that.current === 0 && that.pages === 1 && that.leftRight === 1) {
        ...
        console.log(
          that.tableList,
          that.current,
          that.tableList[that.current].id
        );
        that.infoId = that.tableList[that.current].id;
        console.log(that.infoId);
        that.init();
      ...
      that.$api
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            if (
              Number(res.data.data.isreport) === 1 &&
              Number(res.data.data.isreport_status) === 0
            ) {
              that.formData.sta = 1;
            } else if (
              Number(res.data.data.isreport) === 1 &&
              Number(res.data.data.isreport_status) === 1
            ) {
              that.formData.sta = 2;
            } else if (
              Number(res.data.data.isreport) === 0 &&
              Number(res.data.data.isreport_status) === 0
            ) {
              that.formData.sta = 3;
            }
