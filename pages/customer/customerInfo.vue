<template>
  <view class="fundApplication pageUniapp">
    ...
    <view
      ...
      :style="{
        zIndex: 102,
        transform: `translate${prePage}`,
        transition: `transform ${showAnimation ? turnPageTime : 0}s`,
      }"
    >
      <view class="from">
        <view class="from-content">
          <view class="left"> 客户名<text class="text"></text> </view>
          <view class="right right-choice1">
            <view class="one">
              <input
                type="text"
                class="input-style"
        ...
        <view class="from-content">
          <view class="left"> 区域<text class="text"></text> </view>
          <view class="right">
            <view class="select-content">
              ...
              >
                <view
                  ...
                  >所在区域</view
                >
                <view class="uni-input" v-if="regionList.length > 0">{{
                  regionList[regionIndex]
                    ? regionList[regionIndex].areaname
                    : ""
                }}</view>
          ...
          <view class="left left2"> 权重<text class="text"></text> </view>
          <view class="right">
            <view class="select-content">
              <picker
                class="picker"
              ...
              >
                <view
                  ...
                  >请选择</view
                >
                <view class="uni-input" v-if="weightList.length > 0">{{
                  weightList[weightIndex] ? weightList[weightIndex].name : ""
                }}</view>
                <i class="iconfont icon-leftArrows"></i>
              </picker>
            </view>
        ...
        <view class="from-content">
          ...
          <view class="right">
            <view class="radio-list">
              <view class="radios" @tap="radioCheck">
                <view class="circle">
                  <text
                    class="i"
        ...
        <view class="from-content">
          ...
          <view class="right">
            <view class="radio-list1">
              <view class="radios" @tap="cusBusinessCheck(1)">
                <view class="circle">
                  <text
                    class="i"
              ...
              <view class="radios" @tap="cusBusinessCheck(2)">
                <view class="circle">
                  <text
                    class="i"
              ...
              <view class="radios" @tap="cusBusinessCheck(3)">
                <view class="circle">
                  <text
                    class="i"
              ...
              <view class="radios" @tap="cusBusinessCheck(4)">
                <view class="circle">
                  <text
                    class="i"
          ...
          <view class="right right-choice">
            <view class="one three-btn">
              ...
              <view
                class="three-btn1"
                @tap="install(0)"
                :class="styleAdd === true ? 'styleAdd' : 'nostyleAdd'"
                >提交</view
              >
            </view>
            <view class="two">
              <view class="list" v-for="(val, j) in sendOutList" :key="j">
                <view class="del">
                  <image
                    ...
                    @tap="deleteTap(val, 0)"
                    v-if="Number(val.status) === 0 || Number(val.status) === 1"
                    style="width: 37upx; height: 32upx; margin-right: 10upx"
                    mode=""
                  ></image>
                </view>
                ...
                <view class="del">
                  <image
                    ...
                    @tap="shareWeixin(val, 0)"
                    v-if="Number(val.status) === 1"
                    style="width: 31upx; height: 31upx"
                    mode=""
                  ></image>
                  <image
                    src="../../static/img/examine.png"
                    v-if="Number(val.status) === 0"
                    @tap="examineTap(val, 0)"
            ...
            <view class="two" style="padding-top: 6upx">
              <view
                class="list"
                v-for="(val, j) in sendOutList2"
                :key="j"
                v-if="sendOutList2.length > 0"
              >
                <view class="del">
                  <image
                    src="../../static/img/del.png"
        ...
        <view class="from-content">
          <view class="left"> 相关文书<text class="text"></text> </view>
          <view class="right right-choice">
            <view class="one three-btn">
              ...
              <view
                class="three-btn1"
                @tap="install(0)"
                :class="styleAdd1 === true ? 'styleAdd' : 'nostyleAdd'"
                >提交</view
              >
            </view>
            <view class="two">
        ...
        <view class="from-content">
          <view class="left"> 电话<text class="text"></text> </view>
          <view class="right right-choice">
            <view class="one">
              <input
                type="text"
              ...
              <view
                class="select-btn"
                @tap="CompleteTap"
                :class="colorComplete === 0 ? 'bg-color' : ''"
                style="box-sizing: border-box; padding: 0 10upx"
                >完整</view
              >
            </view>
          </view>
        ...
        <view class="from-content" v-if="contents.length > 0">
          <view class="left left1"> 往来记录<text class="text"></text> </view>
          <view class="right">
              ...
              <checkbox-group @change="checkboxChange1">
                ...
                >
                  ...
                  <view class="content" v-if="item.intupShow === false">
                    ...
                    <image
                      @tap="contentTap(item, i)"
                      class="img"
                      src="../../static/img/update.png"
                      style="width: 35upx; height: 34upx"
                      mode=""
                    ></image>
                  </view>
                </view>
        ...
        <view class="from-content">
          <view class="left"> 业务方式<text class="text"></text> </view>
          <view class="right">
            <view class="radio-list">
              <view class="radios">
    ...
    >
      <view class="from">
        ...
        <view class="from-content">
          <view class="left"> 来源<text class="text"></text> </view>
          <view class="right">
            <view class="radio-list" @tap="radioCheck">
              <view class="radios">
    ...
    <uni-popup ref="popupMessage" type="center">
      <view class="Business-information">
        <view class="title"> 业务资料 </view>
        <view class="one">
          <input
            type="text"
            class="input-style"
            v-model="keyFile"
            style="padding: 0 10upx; box-sizing: border-box"
          />
            ...
            <t-table>
              ...
              <checkbox-group @change="checkboxChange">
                ...
                >
                  ...
                  <t-td widthStyle="16%" :teshu="true">{{
                    item.dir_name
                  }}</t-td>
                  <t-td widthStyle="69%" :teshu="true">{{
                    item.file_name
                  }}</t-td>
                </t-tr>
              </checkbox-group>
...
export default {
  ...
  methods: {
    radioCheck () {
      Number(this.formData.cus_source) === 1 ? this.formData.cus_source = 0 : this.formData.cus_source = 1
      console.log(this.formData.cus_source)
      this.$forceUpdate()
    },
    // 业务状态取消
    cusBusinessCheck (i) {
      const that = this
      if (Number(that.formData.cus_business_status) === i) {
        that.formData.cus_business_status = 0
      } else {
        that.formData.cus_business_status = i
      }
    },
    // 渠道详情
    relationInfo (val) {
      uni.navigateTo({
        url: "./customerChannelInfo?id=" + val.ditch_id + '&customerId=' + this.infoId + '&key=' + this.key
           + '&userArea=' + this.userArea + '&sts=' + this.sts + '&infoTop=' + this.infoTop + '&pages=' + this.pages
      })
    },
    // 转渠道
    cusToDitchTap() {
    ...
    touchMove(e) {
      ...
      // if (that.leftRight)
      if (this.next && this.nextPageReady) {
        //首次翻下一页之后
      ...
      if (this.pre && this.nextPageReady) {
        //首次翻上一页之后
        console.log(123999);
        // 限制边界
        if (delta < 0) {
    ...
    checkboxChange1(e) {
      ...
      for (var i = 0; i < that.contentList.length; i++) {
        data +=
          that.addTimeTsp(that.contentList[i].addtime) +
          "\n" +
    ...
    shareWeixin(val, i) {
      ...
      if (i === 1) {
        let fileName = val.file_path.substring(
          val.file_path.lastIndexOf(".") + 1
        );
    ...
    init() {
      ...
      that.$api.CusDetailInfoApi({ id: that.infoId }).then((res) => {
        if (res.data.status === 200) {
          ...
          that.relationList = res.data.data.relation
